...
describe('<EmailField/>', () => {
  describe('when pass correct props', () => {
    describe('and props validation.isError is true', () => {
      ...
      let BaseElement
      beforeEach(() => {
        const { getByTestId, baseElement } = render(<EmailField validation={{ isError: true }} saveEmail={jest.fn()} />)
    ...
    describe('and props validation.isError is false', () => {
      ...
      let BaseElement
      beforeAll(() => {
        const { getByTestId, baseElement } = render(<EmailField validation={{ isError: false }} saveEmail={jest.fn()} />)
        GetByTestId = getByTestId
        BaseElement = baseElement
      })
